...
void* calloc(size_t nmemb, size_t size)
{
    ...
    // 使用write是为了解决递归调用的问题
    char buffer[100];
    (void) sprintf(buffer, "calloc (%u, %u), return %p , used size = %u\n", 
        (unsigned int)nmemb, (unsigned int)size, user_ptr, (unsigned int)total_used_size);
    (void) write(mem_fp->_fileno, buffer, strlen(buffer));
...
void * malloc(size_t size) {
    ...
    // 使用write是为了解决递归调用的问题
    char buffer[100];
    (void) sprintf(buffer, "malloc (%u), return %p , used size = %u\n", 
        (unsigned int)size, user_ptr, (unsigned int)total_used_size);
    (void) write(mem_fp->_fileno, buffer, strlen(buffer));
...
void Print_Node_Action(int mode, size_t size, struct malloc_list_node* option) {
    ...
    if (mode == 3 && option->info_str != NULL) {
        ...
        (void) sprintf(buffer, "  -Malloc expires: %s", option->info_str);
    }
    else if (mode == 2) {
